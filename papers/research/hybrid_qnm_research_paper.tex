...
Can non-Hermitian black-hole quasinormal-mode (QNM) resonance conditions be handled through a Hermitian residual minimization workflow in a concrete quantum-deformed spacetime? This paper studies that question for Kazakov--Solodukhin (KS) quantum-deformed Schwarzschild black holes. After compactifying the radial equation and factoring the QNM asymptotics, Chebyshev collocation gives a polynomial operator \(P_N(\omega)\bm{u}=0\). For each trial frequency the method forms the Hermitian positive-semidefinite residual \(R_N(\omega)=P_N(\omega)^\dagger P_N(\omega)\), searches for minima of \(\sigma_{\min}(P_N(\omega))\), and accepts candidates after convergence checks and Leaver-style continued-fraction validation. The workflow is presented as a KS-specific implementation grounded in existing singular-value and pseudospectrum ideas, not as a claim to invent residual singular-value theory. The Schwarzschild scalar \(\ell=2\) fundamental mode is recovered at \(N=96\) with relative error \(1.462\times10^{-10}\). The validated catalogue covers scalar and lapse-deformed axial-gravitational \(\ell=2,3,4\) modes through the second overtone. Across \(a/M=0,0.2,0.5,1.0\), sampled branches soften monotonically. For scalar \(\ell=2,n=0\), the \(a/M=1\) endpoint shifts are \(-7.29\%\) in oscillation frequency, \(-2.59\%\) in damping magnitude, and \(-4.83\%\) in quality factor.
\end{quote}
\vspace{1.0em}
]
\thispagestyle{empty}
...
\end{equation}
For \(a=0\), Eq.~\eqref{eq:reggewheelerpotential} reduces to the standard Schwarzschild axial gravitational Regge--Wheeler potential.
...
\label{eq:timedomain}
\end{equation}
The computational domain is \(x/M\in[-250,500]\), with \(\Delta x=0.1M\), \(\Delta t=0.05M\), \(t_{\max}=320M\), and observation point \(x_{\mathrm{obs}}=20M\). A Gaussian pulse \(\Psi(0,x)=\exp[-(x/4M)^2]\) is evolved with first-order outgoing boundary conditions. The interval \(60M\leq t\leq160M\) is fitted by
\begin{equation}
...
\end{equation}
The exponential and power-law factors enforce outgoing behavior at infinity and ingoing behavior at the horizon, while \(u(s)\) is regular on the collocation grid. Substitution into Eq.~\eqref{eq:qnmwave} gives a quadratic polynomial eigenvalue problem
\begin{equation}
P_N(\omega)\bm{u}
\equiv
...
\end{equation}
It is Hermitian and positive semidefinite by construction, up to numerical roundoff. The residual norm reported below is the smallest singular value of \(P_N(\omega)\), and the residual-minimized frequency is obtained by locally minimizing this singular value over complex \(\omega\). In the tables below this residual is used as a classical diagnostic together with condition-number estimates; small residuals are not interpreted as precision guarantees without convergence and Leaver validation.
...
The implementation is reproducible from the accompanying code package, which includes the spectral solver, the Leaver-style validation layer, catalogue-generation scripts, generated numerical tables, and automated tests.
...
The source code, generated catalogue tables, and manuscript-supporting figures are available at \url{https://github.com/adoolelomani2026/ks-qnm-spectral-residual-workflow}.

\noindent\textbf{Author contributions.}
Adel H. Al-Yoorby conceived the study, developed the numerical workflow, performed the calculations, analyzed the results, and wrote the manuscript.
