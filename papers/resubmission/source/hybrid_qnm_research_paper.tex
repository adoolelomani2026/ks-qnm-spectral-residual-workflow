...
contains scalar \(\ell=2,3,4\) fundamentals, validated first overtones, and
...
scalar \(n=0\) fundamentals are robust quantitative results; scalar \(n=1\)
branches are validated low-lying overtones; \(n=2\) entries are exploratory
...
closest candidates enter the detailed scoring stage.
...
+10^{-3}\min(\sigma_j/\sigma_{\min},100),
\end{equation*}
where frequency distances are normalized by the relevant frequency magnitude
with a floor of \(0.1\), and the previous eigenvector is barycentrically
interpolated to the new grid before its normalized overlap is evaluated.
...
with treating \(n=1\) as a validated low-lying overtone while retaining a
...
equation can be checked against tabulated Schwarzschild Regge--Wheeler
frequencies, as reported in
Table~\ref{tab:gravitationalcatalogue}.
...
Across the low-lying working catalogue, the worst spectral--Leaver relative difference is \(1.202\times10^{-5}\), occurring for the axial \(\ell=2\), \(n=2\) branch at \(a/M=0.5\).  The \(n=0\) fundamentals are quantitative results, \(n=1\) entries are validated low-lying overtones, and \(n=2\) entries are exploratory branch diagnostics rather than equal-confidence physical predictions.  Higher overtones are outside the scope.
...
Scalar \(n=1\) & Cross-validated at \(N=32\) & Secondary diagnostics; high-\(N\) tracking remains exploratory. \\
...
scalar fundamentals and validated low-lying first overtones from exploratory
...
shift by \(1.92\%\) in the validated \(\omega_0/\omega_1\) ratio; ratios
...
continuation, backward-error, and collocation-independent continued-fraction checks.  The continued-
fraction truncation table makes those settings and their convergence explicit.
The catalogue is deliberately low lying: \(n=0\) fundamentals are robust,
\(n=1\) modes are validated low-lying overtones, \(n=2\) entries are
exploratory diagnostics, and higher overtones are outside the scope.
...
OpenAI Codex assisted with code development, numerical-analysis
