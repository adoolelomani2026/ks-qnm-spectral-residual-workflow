...
quasinormal modes (QNMs) are the resonances of a leaky gravitational potential
barrier.  Their complex frequencies have an immediate physical meaning:
...
\subsection{Reader's guide to the terminology}

Because black-hole spectroscopy combines relativity, wave mechanics, and
numerical linear algebra, we collect the terminology used throughout the paper
before presenting the calculation.

\paragraph{Mode labels and perturbation sectors.}
The integers \(\ell\) and \(m\) label the spherical harmonic
\(Y_{\ell m}\).  The multipole number \(\ell\) measures angular structure:
\(\ell=0\) is spherically symmetric, \(\ell=1\) is dipolar, and \(\ell=2\) is
quadrupolar.  Spherical symmetry makes the frequency independent of \(m\), so
we suppress that label.  The integer \(n\) orders modes with the same \(\ell\)
by damping.  The least-damped mode is the \emph{fundamental}, \(n=0\); modes
with \(n\geq1\) are \emph{overtones}.  ``Low-lying'' means the fundamental and
first few overtones, which lie closest to the real-frequency axis and usually
dominate the early ringdown.  The \emph{scalar sector} is a massless test field
obeying the Klein--Gordon equation: it probes the geometry without perturbing
the effective source that supports it.  The \emph{axial}, or odd-parity,
sector is the part of a metric perturbation that changes sign as
\((\theta,\phi)\mapsto(\pi-\theta,\phi+\pi)\) by the factor
\((-1)^{\ell+1}\).

\paragraph{Geometry, potential, and boundary conditions.}
The metric tells clocks and radial distances how to respond to position.  Its
\emph{lapse} \(f_a(r)\) vanishes at the event horizon, the one-way surface from
which an outward signal cannot return.  The \emph{tortoise coordinate}
\(r_*\) stretches the finite-radius horizon to \(r_*\to-\infty\) and sends
spatial infinity to \(r_*\to+\infty\); this makes the radial equation look
like a one-dimensional wave equation.  Its \emph{effective potential} is not a
new material force.  It is the combination of spacetime curvature and angular
momentum that scatters the radial wave.  A QNM is ingoing at the horizon and
outgoing at infinity, meaning that energy leaves the exterior region through
both ends and no wave is sent in from either boundary.

\paragraph{Complex frequency and spectroscopy.}
With time dependence \(e^{-i\omega t}\), a damped mode has
\(\omega=\omega_R-i\alpha\), where \(\omega_R=\operatorname{Re}\omega\) is
the angular oscillation frequency and \(\alpha=-\operatorname{Im}\omega>0\)
is the exponential damping rate.  Its e-folding time is
\(\tau=1/\alpha\).  The quality factor is
\(Q=\omega_R/(2\alpha)\).  During one e-folding time the mode accumulates
\(2Q\) radians, or \(Q/\pi\) oscillation cycles; a larger \(Q\) therefore means
weaker damping relative to the period.  We quote
\(M\omega\), which is dimensionless, and compare deformations at fixed \(M\)
so that a change cannot be attributed to changing the mass unit.

\paragraph{WKB and potential-peak intuition.}
The Wentzel--Kramers--Brillouin (WKB) approximation treats the wave locally
near the top of a slowly varying barrier.  At leading order,
\(\operatorname{Re}\omega\sim\sqrt{V_{\rm peak}}\), while damping depends on
the barrier curvature in the tortoise coordinate.  WKB therefore translates
changes in barrier height and width into physical intuition.  In this paper it
is an interpretation tool only; all quoted frequencies come from the
Chebyshev and continued-fraction calculations.

\paragraph{Gauge invariance and inverse Cowling.}
A coordinate change can alter individual metric-perturbation components
without changing the physical spacetime.  A \emph{gauge-invariant} master
variable is a combination unchanged by those infinitesimal coordinate
changes, so a nonzero master wave cannot be removed by relabelling
coordinates.  Gauge invariance does not specify the dynamics of an unknown
quantum source.  The \emph{inverse-Cowling closure} used here freezes the
odd-parity perturbation of that effective source while allowing the metric to
ring.  It is a stated physical model assumption, not a consequence of gauge
invariance.

\paragraph{Spectral and continued-fraction calculations.}
A \emph{spectral method} approximates a smooth function by global basis
functions rather than by local finite-difference steps.  We use Chebyshev
polynomials and enforce the differential equation at \(N\) special
\emph{collocation points}.  This converts the wave equation into a polynomial
matrix equation in \(\omega\).  A generalized eigensolver returns many
\emph{candidate roots} at once, but finite matrices can also produce
\emph{spurious roots} that move or disappear as \(N\) changes.  The Frobenius
method instead writes the regular radial factor as a power series.  Leaver's
continued fraction selects the series solution satisfying both radiative
boundaries.  Because it uses no Chebyshev grid, agreement provides an
independent discretization check rather than a repeat of the same matrix
calculation.

\paragraph{Residual, backward error, convergence, and continuation.}
For a trial frequency, the \emph{residual} measures how nearly the discretized
wave equation is satisfied.  The smallest singular value
\(\sigma_{\min}[P_N(\omega)]\), obtained from a singular-value decomposition,
is the smallest possible residual over normalized trial wavefunctions.  A
\emph{backward error} divides that defect by the scale of the matrix
coefficients and asks how small a relative change to the discretized equation
would make the trial pair exact.  Neither guarantees a physical QNM by itself.
\emph{Spectral convergence} checks whether the frequency stabilizes as \(N\)
changes.  \emph{Continuation} follows a root in small steps of \(a/M\), using
its frequency and wavefunction to preserve \emph{branch identity}; it prevents
an algorithm from silently jumping from one overtone to another.

\paragraph{Pseudospectrum and why it is used.}
The spectrum lists frequencies where the operator is exactly singular.  The
\emph{pseudospectrum} also maps nearby frequencies where it is nearly
singular.  Equivalently, it shows where small perturbations of a
non-self-adjoint operator could move an eigenvalue.  A narrow residual basin
means the mode is sharply isolated in the chosen finite representation; a
broad basin means a larger neighborhood behaves almost like a solution and the
frequency is locally more sensitive to discretization or operator
perturbations.  We use this diagnostic because QNM boundary conditions make
the problem non-self-adjoint, so a tiny residual at one point does not by
itself reveal how sensitive that point is.  Our plots concern finite
Chebyshev matrices: they compare local sensitivity along the validated scalar
branch, but they do not prove a physical instability or determine a continuum
pseudospectrum.
...
We determine the same physical resonances in two independent frequency-domain
representations.  The Chebyshev calculation describes the radial wave globally
...
singular-value pseudospectral diagnostic around a branch that has already been
validated by the independent continued fraction.  Following the
...
support, rather than replace, the physical conclusion: two independent radial
representations identify the same robust fundamental branches, while the
...
relevant session metadata, was used during June--July 2026 for code generation,
debugging, and refactoring; numerical-analysis scripting; algebraic cross-checks;
literature discovery; manuscript drafting and revision; and formatting. The author executed the calculations,
