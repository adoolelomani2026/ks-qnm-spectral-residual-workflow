...
\subsection{Scalar pseudospectrum diagnostics}

The residual formulation also makes it possible to ask a sharper stability question: how does the local pseudospectrum around a validated QNM branch change under KS deformation? Following the singular-value viewpoint used in black-hole pseudospectrum studies \cite{Jaramillo2021,Cao2024}, define the finite-dimensional relative residual
\begin{equation}
...
Figure~\ref{fig:pseudocontours} shows local contours of \(\log_{10}\eta_N\) around the residual-minimized scalar fundamental frequencies for \(a/M=0,0.2,0.5,1.0\). The computation uses \(N=64\), an \(81\times81\) grid, and a square window of half-width \(0.025\) in both \(\mathrm{Re}(M\omega)\) and \(\mathrm{Im}(M\omega)\). The contour centers agree with the Leaver catalogue frequencies to at worst \(6.816\times10^{-12}\) in relative difference, so the plotted basins are attached to the validated scalar branch rather than to an untracked numerical root.

\begin{figure}[H]
\centering
\includegraphics[width=\linewidth]{../../outputs/figures/scalar_l2_pseudospectrum_contours.png}
\caption{Finite-dimensional scalar \(\ell=2,n=0\) pseudospectrum contours for the relative residual \(\eta_N=\sigma_{\min}(P_N)/\sigma_{\max}(P_N)\) at \(N=64\). White crosses mark the residual-minimized QNM locations. The panels show that the local small-\(\eta_N\) basin broadens as \(a/M\) increases.}
\label{fig:pseudocontours}
\end{figure}

The trend can be quantified without relying only on visual inspection. Let \(Q_p\) denote the \(p\)-quantile of \(\log_{10}\eta_N\) over the local window. At \(N=64\), \(-Q_{10}\) increases from \(9.898\) at \(a/M=0\) to \(10.060\) at \(a/M=1\), a gain of \(0.161\). The median susceptibility \(-Q_{50}\) also increases, from \(9.574\) to \(9.710\). The area fraction satisfying \(\log_{10}\eta_N\le -10\) grows from \(4.36\%\) to \(22.07\%\), a factor of \(5.06\); the endpoint contour touches the local-window boundary, so this area should be treated as a window-limited diagnostic rather than a global contour area. These diagnostics are plotted in Fig.~\ref{fig:pseudosensitivity}.

\begin{figure}[H]
\centering
\includegraphics[width=\linewidth]{../../outputs/figures/scalar_l2_pseudospectrum_sensitivity.png}
\caption{Scalar \(\ell=2,n=0\) pseudospectral sensitivity diagnostics at \(N=64\). Left: quantile-based susceptibility measures increase with \(a/M\). Right: fixed-threshold contour area fractions also grow; the deepest endpoint contour is window-limited and is used only as a local diagnostic.}
\label{fig:pseudosensitivity}
\end{figure}

Because absolute pseudospectrum contour levels depend on resolution and normalization, the robust claim is not equality of \(\epsilon\)-contours across \(N\). The finite-resolution check in Fig.~\ref{fig:pseudoresolution} instead tests whether the sign of the susceptibility trend is stable. The \(a/M=0\) to \(a/M=1\) gain in \(-Q_{10}\) is \(0.097\), \(0.130\), and \(0.161\) for \(N=32,48,64\), respectively. Thus, within the present residual normalization, KS deformation makes the local scalar fundamental branch more pseudospectrally sensitive as it softens in frequency.

\begin{figure}[H]
\centering
\includegraphics[width=0.82\linewidth]{../../outputs/figures/scalar_l2_pseudospectrum_resolution_check.png}
\caption{Finite-\(N\) check for the scalar \(\ell=2,n=0\) pseudospectral trend. The plotted quantity is the increase in \(-Q_{10}(\log_{10}\eta_N)\) relative to the Schwarzschild value at the same \(N\). The monotonic increase with \(a/M\) is stable for \(N=32,48,64\), although the absolute contour levels remain resolution dependent.}
\label{fig:pseudoresolution}
\end{figure}

\subsection{Broader scalar stress test}
...
Figure~\ref{fig:prlheatmap} summarizes the endpoint gain in the quantile susceptibility \(-Q_{10}(\log_{10}\eta_N)\) at \(N=64\). All active branches have positive endpoint gain, but this is not a universal-instability result. Only 8 of the 14 active oscillatory branches satisfy the stricter usable/supporting criteria used in the stress test. Three branches are nonmonotonic in \(a/M\) at \(N=64\): \((\ell,n)=(0,1),(1,2),(2,2)\). Two endpoint Leaver checks fail to converge in the exploratory overtone sector, and the largest successful exploratory endpoint discrepancy is \(8.244\times10^{-2}\). A mode-pair scan found no exceptional-point-like candidates: no pair simultaneously showed small normalized frequency separation and near-coalescent mode-shape overlap.

\begin{figure}[H]
\centering
\includegraphics[width=\linewidth]{../../outputs/figures/prl_instability_central_heatmap.png}
\caption{Broad scalar finite-\(N\) stress test for a possible universal pseudospectral-instability claim. Entries show the endpoint gain in \(-Q_{10}(\log_{10}\eta_N)\) from \(a/M=0\) to \(a/M=1\) at \(N=64\). Stars mark caution or exploratory branches. Positive endpoint gain is widespread, but the starred and nonmonotonic branches prevent a universal claim.}
\label{fig:prlheatmap}
\end{figure}

The cleanest broad result is more modest: scalar fundamental branches show positive endpoint gain through the finite-\(N\) check. Figure~\ref{fig:prlrobustness} shows that the endpoint susceptibility gain for \(n=0\) remains positive for \(\ell=0,\ldots,4\) through the tested sizes, including \(N=96\). This supports the interpretation that KS softening is accompanied by increased local finite-\(N\) pseudospectral sensitivity on scalar fundamental branches. It does not establish a continuum pseudospectrum theorem, a universal overtone law, or a general discovery-level claim about quantum black holes.

\begin{figure}[H]
\centering
\includegraphics[width=0.86\linewidth]{../../outputs/figures/prl_instability_robustness.png}
\caption{Fundamental-branch finite-\(N\) robustness check from the broader scalar stress test. The plotted quantity is the endpoint gain in \(-Q_{10}(\log_{10}\eta_N)\) from \(a/M=0\) to \(a/M=1\). The gain is positive for scalar \(\ell=0,\ldots,4\) through the tested spectral sizes, but this robustness is limited to the finite-dimensional residual normalization used here.}
\label{fig:prlrobustness}
\end{figure}

\subsection{Branch dependence of quantum-induced pseudospectral sensitivity}

The stress-test data are more informative when universal amplification is treated as the null hypothesis rather than as the target conclusion.  Each active scalar branch was therefore reclassified using only the generated branch verdicts, endpoint Leaver checks, finite-\(N\) spread, and \(N=64\) susceptibility trend.  A branch is called robustly increasing when it is usable, monotonic in \(a/M\), positive at every tested \(N\), and has positive endpoint gain.  A weakly increasing branch has the same positive finite-\(N\) trend but carries a caution flag, usually because the local deep contour touches the plotting window or because it is a second overtone.  Nonmonotonic branches have positive endpoint gain but fail monotonicity in \(a/M\).  Numerically inconclusive branches have failed endpoint validation or large cross-\(N\) center spread.  The resulting classification is shown in Table~\ref{tab:branchsensitivityclasses}.
...
\begin{tabularx}{\columnwidth}{p{0.25\columnwidth}p{0.34\columnwidth}X}
\toprule
Class & Branches \((\ell,n)\) & Interpretation \\
\midrule
Robustly increasing & \(\begin{array}{@{}l@{}}(1,0),(1,1),(2,0),(2,1),\\(3,0),(3,1),(4,0),(4,1)\end{array}\) & Monotonic positive gain with usable finite-\(N\) status. \\
Weakly increasing & \(\begin{array}{@{}l@{}}(0,0),(3,2),(4,2)\end{array}\) & Positive monotonic gain, but with low-\(\ell\) or overtone caution flags. \\
Nonmonotonic & \((0,1)\) & Positive endpoint gain but nonmonotonic susceptibility over the sampled \(a/M\) grid. \\
Numerically inconclusive & \((1,2),(2,2)\) & Endpoint gain is positive, but endpoint validation or cross-\(N\) tracking is not strong enough for interpretation. \\
\bottomrule
\end{tabularx}
\caption{Branch-level classification of the scalar finite-\(N\) pseudospectrum stress test.  The classification is generated by \texttt{scripts/analyze\_branch\_sensitivity.py} from the existing stress-test tables.}
\label{tab:branchsensitivityclasses}
\end{table}

Several tested quantities correlate with the endpoint susceptibility gain, but they do not have equal interpretive status.  The strongest continuous correlate is the growth of the generalized-eigenvalue condition indicator, \(\log_{10}[\kappa(a/M=1)/\kappa(0)]\), with Pearson coefficient \(r=0.961\) against the \(N=64\) \(-Q_{10}\) gain.  This is useful but not surprising: \(\kappa\) is itself a finite-dimensional non-Hermitian sensitivity diagnostic of the same discretized operator.  Overtone load,
\begin{equation}
\chi_{\ell n}=\frac{n}{\ell+1/2},
\end{equation}
is the simplest empirical branch separator in the current sample: all reliable increasing branches have \(\chi_{\ell n}\leq2/3\), while the nonmonotonic or numerically inconclusive branches have larger \(\chi_{\ell n}\).  The endpoint damping-rate shift gives an equivalent finite-sample split: reliable increasing branches have at least a \(1.96\%\) decrease in \(-\mathrm{Im}\,\omega\), whereas the fragile branches do not.  These thresholds should not be read as derived laws; they are empirical descriptors of a 14-branch scan.

The remaining correlations are weaker or less predictive.  The susceptibility gain increases with overtone index and decreases with \(\ell\), consistent with overtones being more deformation-sensitive but less robust.  It anticorrelates with baseline quality factor and nearest-neighbor eigenvalue distance.  Direct physics-facing deformation measures are not enough by themselves: correlations with real-frequency softening, damping shift, potential-height softening, potential-curvature softening, and quality-factor shift are substantially weaker than the condition-growth correlation.  No exceptional-point-like branch coalescence is found.  Thus the branch-resolved conclusion is deliberately modest: KS deformation increases finite-\(N\) residual sensitivity most reliably for scalar fundamentals and first overtones with moderate overtone load, while the high-load overtone sector remains branch dependent and open.

\FloatBarrier
\section{Discussion}

The central physical pattern is the monotonic softening of the validated KS spectrum. Increasing \(a/M\) lowers both the oscillation frequency and the absolute damping rate for every sampled scalar and lapse-deformed axial branch in the catalogue. The scalar \(\ell=2\) fundamental mode is the safest interpretation of this pattern: at \(a/M=1\), its real frequency is lower by \(7.29\%\), its damping magnitude is lower by \(2.59\%\), and its quality factor is lower by \(4.83\%\). The pseudospectrum analysis adds a second numerical observation: in the same scalar branch, the local relative-singular-value basin broadens with KS deformation, indicating greater finite-\(N\) spectral sensitivity in the residual operator. The broader scalar stress test refines that statement rather than universalizing it. Sensitivity growth is robust for scalar fundamentals and first overtones with moderate overtone load, weaker or branch-sensitive for low-\(\ell\) and second-overtone cases, and not governed by a demonstrated physical scaling law.
...
The pseudospectrum diagnostics are similarly bounded. They are computed from finite Chebyshev matrices \(P_N(\omega)\), not from a rigorously controlled infinite-dimensional KS wave operator. The use of \(\eta_N=\sigma_{\min}/\sigma_{\max}\) reduces simple matrix-scale effects but does not remove all discretization or norm choices. The fixed-threshold contour areas are therefore secondary diagnostics; the stronger statement is that the quantile-based local susceptibility increases monotonically with \(a/M\) for \(N=32,48,64\) on the scalar \(\ell=2,n=0\) branch. The broader scalar stress test is deliberately more conservative: positive endpoint susceptibility gain is widespread, but the branch classification separates robustly increasing, weakly increasing, nonmonotonic, and numerically inconclusive cases. Condition-number growth tracks the gain magnitude, and the empirical overtone-load threshold \(\chi_{\ell n}=n/(\ell+1/2)\leq2/3\) separates the reliable increasing cases in the current sample, but neither observation is a derived continuum scaling law. No exceptional-point claim is supported.
...
\end{center}
The repository contains the Chebyshev spectral solver, the Leaver-style validation layer, catalogue-generation scripts, pseudospectrum diagnostics, generated tables, manuscript-supporting figures, and automated tests. The default validation entry point is the test suite; full catalogue regeneration is driven by the main algorithm script in \texttt{scripts/}. The pseudospectrum data and figures in Sec.~5.9 are regenerated with \texttt{python scripts/analyze\_pseudospectrum.py}. The broader scalar stress test in Sec.~5.10 is regenerated with \texttt{python scripts/run\_prl\_instability\_scan.py}, and the branch-dependence classification in Sec.~5.11 is regenerated with \texttt{python scripts/analyze\_branch\_sensitivity.py}. These commands write the scalar pseudospectrum grids, branch verdicts, endpoint Leaver checks, branch-sensitivity classes, diagnostic reports, and figures under \path{outputs/results/} and \path{outputs/figures/}.
