...
contains robust scalar \(\ell=2,3,4\) fundamentals, cross-validated low-lying
first overtones at \(N=32\), and
...
\bottomrule
\end{tabular}
\caption{Reported scalar comparison. Fundamental branches are reported at \(N=96\); the cross-validated low-lying first overtones at \(N=32\) are frozen on that reference grid. Displayed decimals are for branch identification and table readability; precision claims are set by convergence and continued-fraction validation, not by residual size alone. Tracked high-\(N\) overtone rows are retained as exploratory diagnostics rather than used as headline results.}
