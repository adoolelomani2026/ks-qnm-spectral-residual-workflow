...
The convergence of the direct spectral fundamental branches with Chebyshev size is shown in Fig.~\ref{fig:spectralconvergence}. The plotted error is the complex difference from the \(N=96\) value at the same \(a/M\). It falls from the \(10^{-8}\) scale at \(N=16\) to \(10^{-10}\) or below by \(N=24\), after which the curves sit on the high-\(N\) numerical plateau expected from the ill-conditioned residual pencils.
...
Representative lapse-deformed axial-gravitational \(\ell=2\) trajectories over the deformation grid are shown in Fig.~\ref{fig:gravitationaltrajectory}.
...
The sign of the quantile trend is stable under the tested spectral sizes. As shown in Fig.~\ref{fig:pseudoresolution}, the \(a/M=0\) to \(a/M=1\) gain in \(-Q_{10}\) is \(0.097\), \(0.130\), and \(0.161\) for \(N=32,48,64\), respectively. Thus, within the present residual normalization, scalar fundamental softening is associated with increasing local pseudospectral sensitivity.
